  ...
  foreach ($b in $blocks) {
    ...
    foreach ($unit in $data) {
      foreach ($w in $unit.words) {
        ...
        # 2) word field sanity: 字母+常用标点+法语借词音符(cafe/facade类)
        if ($word -notmatch "^[A-Za-z(\[][A-Za-z0-9 '\-\.&/()=\[\],$([char]0x00E9)$([char]0x00E8)$([char]0x00E7)$([char]0x00E0)$([char]0x00EA)]*$") { $issues.Add("$loc word field suspicious: '$word'") }
