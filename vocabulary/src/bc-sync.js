...
// 可安全推送的档：结构合法、非空、非疑似损坏、体积合规；否则返回 null
function syncPushableRaw(id){
  const raw = syncBookRaw(id);
  if (!raw) return null;
  if (raw.length > 1200000){ syncLastErr = '进度体积超限，暂无法同步'; return null; }
  const d = syncParse(raw);
  if (!syncValidArchive(d)) return null;
  if (syncLooksPoisoned(d)) return null;
  if (Object.keys(d.w).length === 0 && !(d.xp > 0) && Object.keys(d.ck || {}).length === 0) return null; // 纯空档不推
  return raw;
}
...
function syncRestoreBackup(key){
  try {
    ...
    ask('用这份备份覆盖《' + ((BOOKS.find(b=>b.id===book)||{}).name || book) + '》当前进度吗？\n覆盖前会把当前进度存为新备份。', ()=>{
      ...
      toast('✔ 已恢复备份' + (syncAuth ? '，稍后自动同步到云端' : ''));
      if (syncAuth) syncSchedulePush();
...
async function syncPushBook(id, isRetry){
  ...
  if (!raw){ m.dirty = false; syncSaveLocal(); return false; }   // 不可推送(空/损坏/超限) → 放弃这次，不再空转
  const seq0 = m.seq || 0;
...
async function syncApplyCloud(id, r){
  ...
  if (id === curBookId){
    // 训练或任何弹层界面中不替换当前进度；词书数据没加载好也不替换
...
function syncBoot(){
  ...
  document.addEventListener('visibilitychange', ()=>{
    if (document.visibilityState === 'hidden'){
      // 离开页面前尽力推送。keepalive 有 64KB 体积上限，大档跳过——
      // 脏标记已落盘，下次打开会自动补传，不会丢。
      if (!syncAuth) return;
      for (const id of Object.keys(syncMeta)){
        if (!syncMeta[id] || !syncMeta[id].dirty) continue;
        const raw = syncPushableRaw(id);
        if (raw && raw.length < 55000){
          syncApi('sync', { method:'POST', keepalive:true, body:{
            book_id:id, data:raw, updated_at:new Date().toISOString(),
            base_server_at:(syncMeta[id]||{}).serverAt || null, device:syncDevice() } });
        }
      }
